...
export function runStorageContractSuite(harness: AdapterHarness): void {
  describe(`storage contract suite: ${harness.name}`, () => {
    ...
    it('round-trips an empty object', async () => {
      ...
      const fetched = await adapter.get('empty');
    ...
    it('round-trips a small binary object', async () => {
      ...
      const fetched = await adapter.get('small.bin');
    ...
    it('round-trips a large binary object', async () => {
      ...
      const fetched = await adapter.get('large.bin');
      const bytes = concatBytes(await collect(fromWeb(fetched.body)));
    ...
    it('accepts bytes in varying chunk sizes', async () => {
      ...
      const fetched = await adapter.get('chunked.bin');
      const bytes = concatBytes(await collect(fromWeb(fetched.body)));
      expect(bytesEqual(bytes, fixture)).toBe(true);
    });
    ...
    it('exposes a size for every stored object', async () => {
      ...
      const fetched = await adapter.get('sized.bin');
    ...
    it('reports existence and non-existence', async () => {
      ...
      await expect(adapter.exists('exists.bin')).resolves.toBe(true);
      await expect(adapter.exists('missing.bin')).resolves.toBe(false);
    });

    it('get on a missing key fails', async () => {
      await expect(adapter.get('does-not-exist')).rejects.toThrow(/not found/i);
    });
    ...
    it('same-key puts never silently mix byte streams (first write wins, later writes ignored)', async () => {
      ...
      const after = concatBytes(await collect(fromWeb((await adapter.get('a.bin')).body)));
    ...
    if (!harness.filesystemOnly) {
      it('a failed source stream leaves no readable object', async () => {
        ...
        await expect(adapter.get('broken.bin')).rejects.toThrow(/not found/i);
        await expect(adapter.exists('broken.bin')).resolves.toBe(false);
      });
    }
    ...
    it('concurrent puts of distinct keys both succeed', async () => {
      ...
      await expect(adapter.exists('conc-a.bin')).resolves.toBe(true);
      await expect(adapter.exists('conc-b.bin')).resolves.toBe(true);
    });
    ...
    it('concurrent puts of the same key produce exactly one committed object', async () => {
      ...
      const fetched = await adapter.get('same.bin');
      expect(fetched.size).toBe(fixture.byteLength);
      const bytes = concatBytes(await collect(fromWeb(fetched.body)));
      expect(bytesEqual(bytes, fixture)).toBe(true);
    });
    ...
    if (harness.corruptObject) {
      it('get reports an error after the object is removed', async () => {
        ...
        await expect(adapter.get('remove-me.bin')).rejects.toThrow(/not found/i);
        await expect(adapter.exists('remove-me.bin')).resolves.toBe(false);
      });
    }
